...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    api.get('/auth/me').then(({ data }) => { setUser(data.data.user); localStorage.setItem(USER_KEY, JSON.stringify(data.data.user)) }).catch(() => { localStorage.removeItem(TOKEN_KEY); localStorage.removeItem(USER_KEY); setToken(null); setUser(null) }).finally(() => setLoading(false))
    return undefined
  }, [token])
  ...
  const logout = useCallback(async () => { try { if (token) await api.post('/auth/logout') } finally { localStorage.removeItem(TOKEN_KEY); localStorage.removeItem(USER_KEY); setToken(null); setUser(null) } }, [token])
  const value = useMemo(() => ({ login, register, logout, user, isAuthenticated: Boolean(token && user), loading }), [token, user, loading, login, register, logout])
