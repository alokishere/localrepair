...
function validateCredentials(
  ...
  if (!isRegistration && typeof password !== "string")
    errors.push({ field: "password", message: "Password is required" });
  if (
    phone !== undefined &&
    (typeof phone !== "string" || !PHONE_PATTERN.test(phone.trim()))
  )
    errors.push({ field: "phone", message: "Enter a valid phone number" });
  if (isRegistration && !ALLOWED_REGISTRATION_ROLES.includes(role))
    errors.push({
      field: "role",
      message: "Role must be CUSTOMER or TECHNICIAN",
    });
  if (errors.length)
    throw validationError("Please correct the highlighted fields", errors);
}
