...
function publicProfile(profile) {
  ...
  return {
    ...
    serviceCategories: (profile.serviceCategories || []).map((category) => ({
      id: category._id,
      name: category.name,
      slug: category.slug,
      icon: category.icon,
...
const profileQuery = () =>
  TechnicianProfile.find({ verificationStatus: "VERIFIED", isAvailable: true })
...
async function listTechnicians(req, res, next) {
  try {
    ...
    const category = categoryId
      ? parseObjectId(categoryId, "categoryId")
      : null;
    const parsedLimit = Number(limit);
    if (!Number.isInteger(parsedLimit) || parsedLimit < 1 || parsedLimit > 50)
      throw badRequest("limit must be an integer between 1 and 50", [
        { field: "limit", message: "Use a limit from 1 to 50" },
      ]);
    ...
    return res.json({
      ...
      data: {
        technicians: profiles.map((profile) => publicCard(profile)),
...
async function getTechnician(req, res, next) {
  try {
    ...
    if (!profile)
      return res
        .status(404)
        .json({ success: false, message: "Technician not found", errors: [] });
    const reviews = await Review.find({
      technicianId: profile.userId._id || profile.userId,
    })
      .sort({ createdAt: -1 })
      .limit(5)
      ...
      data: {
        ...
        reviews: reviews.map((review) => ({
          ...
          customer: review.customerId
            ? { name: review.customerId.name, avatar: review.customerId.avatar }
            : null,
        })),
      },
    });
...
async function nearbyTechnicians(req, res, next) {
  try {
    ...
    if (
      ...
      throw badRequest("Valid lng, lat, and radius are required", [
        {
          field: "location",
          message: "lng must be -180..180, lat -90..90, radius 0..500",
        },
      ]);
    }
    const category = categoryId
    ...
    return res.json({
      success: true,
      message: "Nearby technicians retrieved",
      data: {
        technicians: profiles.map((profile) =>
          publicCard(profile, distance(profile)),
        ),
        radiusKm,
